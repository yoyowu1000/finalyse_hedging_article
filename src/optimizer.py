...
def calculate_present_value(cashflows: list[tuple[float, float]], 
...
class HedgingOptimizer:
    ...
    def duration_matching(self) -> dict:
        ...
        liability_pv = sum(l.amount * self.yield_curve.get_discount_factor(l.time_years)
                          for l in self.liabilities)
        
        if liability_pv == 0:
            raise ValueError("Liability present value is zero")
        
        liability_duration = sum(l.amount * l.time_years * 
                               self.yield_curve.get_discount_factor(l.time_years)
                               for l in self.liabilities) / liability_pv
        
        # Bond metrics
        bond_pvs = np.array([calculate_present_value(calculate_bond_cashflows(bond), 
                                                    self.yield_curve) 
                           for bond in self.bonds])
        bond_durations = np.array([calculate_duration(bond, self.yield_curve) 
                                 for bond in self.bonds])
        ...
        return {
            ...
            'portfolio_duration': (np.dot(result.x * bond_pvs, bond_durations) / 
                                 np.dot(result.x, bond_pvs)) if result.success and np.dot(result.x, bond_pvs) > 0 else 0,
            'bond_allocations': [(bond, qty) for bond, qty in zip(self.bonds, result.x) 
                               if qty > 0.01]
        }
    ...
    def cash_flow_matching(self) -> dict:
        ...
        """
        # Create time grid
        liability_times = sorted(set(l.time_years for l in self.liabilities))
        ...
        for i, t in enumerate(all_times):
            # Liability at time t
            b[i] = sum(l.amount for l in self.liabilities if abs(l.time_years - t) < 0.001)
        ...
        c = np.array([calculate_present_value(calculate_bond_cashflows(bond), 
                                            self.yield_curve) 
                     for bond in self.bonds])
        ...
        return {
            ...
            'bond_allocations': [(bond, qty) for bond, qty in zip(self.bonds, quantities) 
                               if qty > 0.01]
        }