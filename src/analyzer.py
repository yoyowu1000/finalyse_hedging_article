...
class HedgingAnalyzer:
    ...
    def __init__(self, liabilities: list[Liability], 
                 bonds: list[Bond], 
    ...
    def create_cashflow_comparison(self, save_path: Optional[str] = None) -> plt.Figure:
        ...
        for t in all_times:
            # Liability cashflow
            liability_cf = sum(l.amount for l in self.liabilities 
                             if abs(l.time_years - t) < 0.001)
            liability_cfs.append(liability_cf)
            
            # Asset cashflow
            asset_cf = 0
            for bond, qty in zip(self.bonds, self.quantities):
                bond_cf = sum(cf for time, cf in calculate_bond_cashflows(bond)
                            if abs(time - t) < 0.001)
        ...
        bars1 = ax1.bar(x - width/2, liability_cfs, width, label='Liabilities', 
                        alpha=0.8, color='#e74c3c')
        bars2 = ax1.bar(x + width/2, asset_cfs, width, label='Hedging Portfolio', 
        ...
        # Mismatch analysis
        mismatches = [asset - liability for asset, liability in zip(asset_cfs, liability_cfs)]
        ...
        ax2.set_title('Hedging Effectiveness: Cash Flow Mismatches', fontsize=14, fontweight='bold')
        ax2.set_xticks(x)
    ...
    def sensitivity_analysis(self, 
        ...
        """
        # Base case values
        base_liability_pv = sum(l.amount * self.yield_curve.get_discount_factor(l.time_years)
                               for l in self.liabilities)
        
        base_asset_pv = sum(qty * calculate_present_value(calculate_bond_cashflows(bond), 
                                                         self.yield_curve)
                           for bond, qty in zip(self.bonds, self.quantities))
        ...
        for shift in yield_shifts:
            ...
            # Calculate new PVs
            liability_pv = sum(l.amount * shifted_curve.get_discount_factor(l.time_years)
                              for l in self.liabilities)
            
            asset_pv = sum(qty * calculate_present_value(calculate_bond_cashflows(bond), 
                                                        shifted_curve)
                          for bond, qty in zip(self.bonds, self.quantities))
            ...
            if base_liability_pv != 0:
                ...
                liability_change = (liability_pv - base_liability_pv) / base_liability_pv
        ...
        # PV changes
        ax1.plot(yield_shifts * 100, (np.array(liability_pvs) / base_liability_pv - 1) * 100, 
                'r-', label='Liabilities', linewidth=2.5, marker='o')
        ax1.plot(yield_shifts * 100, (np.array(asset_pvs) / base_asset_pv - 1) * 100, 
                'b-', label='Hedging Portfolio', linewidth=2.5, marker='s')
        ax1.set_xlabel('Yield Shift (basis points)', fontsize=12)
        ax1.set_ylabel('PV Change (%)', fontsize=12)
        ax1.set_title('Interest Rate Sensitivity Analysis', fontsize=14, fontweight='bold')
        ax1.legend(fontsize=12)
        ax1.grid(True, alpha=0.3)
        ...
        # Tracking error
        ax3.plot(yield_shifts * 100, np.array(tracking_errors) * 100, 
                'purple', linewidth=2.5, marker='^')
