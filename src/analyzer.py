...
"""
    ...
    def create_portfolio_comparison(
        ...
        bond_labels = [f"{bond.maturity_years}Y {bond.coupon_rate:.1%}" for bond in self.bonds]
        x = np.arange(len(self.bonds))
        width = 0.35
        
        bars1 = ax1.bar(x - width/2, initial_quantities, width, label='Initial Portfolio', alpha=0.8, color='#ff7f0e')
        bars2 = ax1.bar(x + width/2, optimized_quantities, width, label='Optimized Portfolio', alpha=0.8, color='#2ca02c')
        ...
        ax2.bar(x - width, initial_values, width, label='Initial', alpha=0.8, color='#ff7f0e')
        ax2.bar(x, optimized_values, width, label='Optimized', alpha=0.8, color='#2ca02c')
        ax2.bar(x + width, target_values, width, label='Target (Liability)', alpha=0.8, color='#e74c3c')
        
        ax2.set_ylabel('Value', fontsize=12)
        ax2.set_title('Portfolio Metrics Comparison', fontsize=14, fontweight='bold')
        ax2.set_xticks(x)
        ax2.set_xticklabels(metrics)
        ax2.legend(fontsize=10)
        ax2.grid(True, alpha=0.3)
        
        # 3. Tracking error improvement
        ...
        ax3.bar(x - width/2, initial_errors, width, label='Initial', alpha=0.8, color='#ff7f0e')
        ax3.bar(x + width/2, optimized_errors, width, label='Optimized', alpha=0.8, color='#2ca02c')
        ...
        for i, (initial, optimized) in enumerate(zip(initial_errors, optimized_errors)):
            if initial > 0:
                ...
                ax3.text(i, max(initial, optimized) * 1.1, 
...
Initial Portfolio (Maturity Bucketing):
  - PV: €{initial_pv:,.0f}k
  - Duration: {initial_duration:.2f} years
  - PV Error: {initial_pv_error:.2f}%
  - Duration Error: {initial_duration_error:.3f} years

Optimized Portfolio ({optimization_type}):
  - PV: €{optimized_pv:,.0f}k
  - Duration: {optimized_duration:.2f} years
  - PV Error: {optimized_pv_error:.2f}%
  - Duration Error: {optimized_duration_error:.3f} years

Improvement:
  - PV Error: {(initial_pv_error - optimized_pv_error) / initial_pv_error * 100 if initial_pv_error > 0 else 0:.0f}% reduction
  - Duration Error: {(initial_duration_error - optimized_duration_error) / initial_duration_error * 100 if initial_duration_error > 0 else 0:.0f}% reduction
"""
        
        fig.text(0.02, 0.02, summary_text, fontsize=10, fontfamily='monospace',
                bbox=dict(boxstyle='round', facecolor='wheat', alpha=0.5))
        
        plt.suptitle(f'Portfolio Optimization Analysis: {optimization_type}', fontsize=16, fontweight='bold')
